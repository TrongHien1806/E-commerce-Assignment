...
Tài liệu này tổng hợp API thực tế từ source code backend hiện tại.
...
## GET `/orders`
Lấy list order của user hiện tại.

## GET `/orders/:orderId`
Lấy chi tiết order của user hiện tại.
...
## 7) Tracking (`/tracking`) - Auth bắt buộc
...
## 9) Gợi ý tích hợp frontend (quan trọng)
...
## 10) Lưu ý hiện trạng code backend

- `POST /reviews`: logic kiểm tra “verified purchase” hiện đang dò `order.items.itemType`, trong khi schema `OrderItem` hiện không có field `itemType`.
  - Có thể dẫn tới không tạo review được dù đã mua.
  - Frontend nên chuẩn bị xử lý lỗi `403` với message `Bạn chỉ có thể đánh giá khi đã mua hoặc sử dụng dịch vụ`.

- `src/index.ts` mount `/orders` 2 lần (không phá API nhưng dư thừa).

---

Nếu cần, mình có thể làm thêm bản **OpenAPI 3.0 (swagger)** từ tài liệu này để frontend import thẳng vào Postman/Insomnia hoặc generate API client tự động.